...
const demoPools: DemoPool[] = [
  {
    ...
    member_count: 12,
    token: "SOL",
    target_tokens: ["BAGS", "SOL"],
    my_contribution: 15.2,
    my_share: 6.2,
    is_active: true,
    votes: [
      { id: "v1", token: "BAGS", votes_for: 8, votes_against: 2, total_voters: 12, voted: 10, status: "active" },
      { id: "v2", token: "SOL", votes_for: 11, votes_against: 1, total_voters: 12, voted: 12, status: "passed" },
    ],
  },
  {
    id: "2",
    ...
    member_count: 28,
    token: "USDC",
    target_tokens: ["USDC", "USDT"],
    my_contribution: 500,
    my_share: 4.0,
    is_active: true,
    votes: [
      { id: "v3", token: "USDT", votes_for: 18, votes_against: 6, total_voters: 28, voted: 24, status: "active" },
    ],
  },
  {
    id: "3",
    name: "Bags Maximalist",
    description: "All-in on BAGS token long term hold",
    total_value: 85000,
    member_count: 45,
    token: "BAGS",
    target_tokens: ["BAGS"],
    my_contribution: 2000,
    my_share: 2.35,
    is_active: true,
    votes: [],
  },
...
const availablePools: DemoPool[] = [
  {
    id: "4",
    name: "DeFi Explorers",
    description: "Exploring new DeFi protocols on Solana",
    total_value: 89.3,
    member_count: 7,
    token: "SOL",
    target_tokens: ["SOL", "BAGS"],
    is_active: true,
  },
  {
    id: "5",
    name: "Micro Cap Gems",
    description: "Small cap token research and investment pool",
    total_value: 3200,
    member_count: 15,
    token: "USDC",
    target_tokens: ["BAGS", "SOL"],
...
const PoolsSection = () => {
  ...
  const [createForm, setCreateForm] = useState({
    name: "",
    description: "",
    token: "SOL" as PoolToken,
    target_tokens: [] as string[],
    min_contribution: "",
    pool_size_limit: "",
  });
  ...
  const toggleTargetToken = (t: string) => {
    setCreateForm((prev) => ({
      ...prev,
      target_tokens: prev.target_tokens.includes(t)
        ? prev.target_tokens.filter((x) => x !== t)
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Button
            ...
            size="sm"
            onClick={() => setView(view === "join" ? "active" : "join")}
          >
          ...
          <Button
            ...
            size="sm"
            onClick={() => setView(view === "create" ? "active" : "create")}
          >
      ...
      <AnimatePresence mode="wait">
        ...
        {view === "create" && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="rounded-2xl border border-primary/30 bg-card p-6 mb-6 relative overflow-hidden"
          >
            <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-accent/5" />
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-display text-lg font-bold text-foreground">Create New Pool</h3>
                <button onClick={() => setView("active")} className="text-muted-foreground hover:text-foreground">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  {allTokens.map((t) => (
                    <button
                      key={t}
                      onClick={() => toggleTargetToken(t)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                        createForm.target_tokens.includes(t)
                          ? tokenColors[t] + " border-current"
                          : "bg-secondary text-muted-foreground border-border hover:border-muted-foreground"
                      }`}
                    >
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <Input
                    type="number"
                    placeholder="0.5"
                    value={createForm.min_contribution}
                    onChange={(e) => setCreateForm({ ...createForm, min_contribution: e.target.value })}
                    className="bg-secondary border-border"
                  />
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Pool Size Limit (members)</label>
                  <Input
                    type="number"
                    placeholder="50"
                    value={createForm.pool_size_limit}
                    onChange={(e) => setCreateForm({ ...createForm, pool_size_limit: e.target.value })}
                    className="bg-secondary border-border"
                  />
                </div>
              </div>
          ...
          >
            ...
            <div className="space-y-3">
              {availablePools.map((pool) => (
                <div
                  key={pool.id}
                  className="rounded-2xl border border-border bg-card p-5 hover:border-primary/40 transition-all duration-300 group relative overflow-hidden"
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <Badge variant="outline" className={`text-xs border ${tokenColors[pool.token]}`}>
                        {pool.token}
                      </Badge>
                    </div>
                    ...
                    <div className="flex items-center gap-4 mb-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        ...
                        {pool.total_value.toLocaleString()} {pool.token}
                      </span>
                      <span className="flex items-center gap-1">
                    ...
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      {pool.target_tokens.map((t) => (
                        <Badge key={t} variant="outline" className={`text-[10px] px-1.5 py-0 border ${tokenColors[t]}`}>
                          {t}
                        </Badge>
                      ))}
                    </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-display text-lg font-bold text-foreground flex items-center gap-2">
                  ...
                  Votes — {selectedPool.name}
                </h3>
                <p className="text-xs text-muted-foreground mt-0.5">Members vote on which tokens the pool should invest in</p>
              </div>
              <button onClick={() => { setView("active"); setSelectedPool(null); }} className="text-muted-foreground hover:text-foreground">
                <X className="w-5 h-5" />
              </button>
            </div>

            {selectedPool.votes && selectedPool.votes.length > 0 ? (
              <div className="space-y-3">
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className={`text-[10px] border ${tokenColors[vote.token]}`}>
                              {vote.token}
                            </Badge>
                          </div>
                          <Badge
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="rounded-xl bg-secondary p-3">
                      ...
                      <p className="text-sm font-bold text-foreground tabular-nums">
                        {pool.my_contribution} {pool.token}
                      </p>
                    </div>
                    <div className="rounded-xl bg-secondary p-3">
