...
import { Plus, UserPlus, Users, Vote, TrendingUp, X, BarChart3, Zap } from "lucide-react";
...
const PoolsSection = () => {
  ...
  const joinPool = (pool: DemoPool) => {
    ...
    toast({ title: "Joined pool", description: `${amount} ${pool.token} committed to ${pool.name}.` });
  };
  ...
  const handleVote = (poolId: string, voteId: string, direction: VoteDirection) => {
    ...
    toast({ title: `Vote ${direction} registered`, description: "The progress bar updated in real time." });
  };
            ...
            <div className="relative z-10">
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-4">
                <div className="grid grid-cols-4 gap-2">
                  {allTokens.map((token) => (
                    <button key={token} type="button" onClick={() => setCreateForm({ ...createForm, token })} className={`rounded-lg px-2 py-2 text-xs font-semibold border transition-all ${createForm.token === token ? "border-primary/60 bg-primary text-primary-foreground" : "border-border bg-secondary text-muted-foreground"}`}>{token}</button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-4 gap-2 mb-5">
                {allTokens.map((token) => (
                  <button key={token} type="button" onClick={() => toggleTargetToken(token)} className={`rounded-lg px-2 py-2 text-xs font-semibold border transition-all ${createForm.target_tokens.includes(token) ? "border-primary/60 bg-primary text-primary-foreground" : "border-border bg-secondary text-muted-foreground"}`}>{token}</button>
                ))}
              </div>
              <Button className="w-full" onClick={createPool}>Create Pool</Button>
            </div>
          </motion.div>
        )}
                ...
                <div key={pool.id} className="rounded-2xl border border-border bg-card p-5 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {pool.target_tokens.map((token) => <Badge key={token} variant="outline" className={`text-[10px] border ${tokenColors[token]}`}>{token}</Badge>)}
                    </div>
                    <div className="flex gap-2">
                  ...
                  <div key={vote.id} className="rounded-2xl border border-border bg-card p-5 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <Badge variant="outline" className="text-[10px] border-primary/30 bg-primary/10 text-accent">{vote.status}</Badge>
                      </div>
                      <div className="mb-3">
                        ...
                        <div className="h-3 rounded-full bg-secondary overflow-hidden flex"><motion.div className="h-full bg-primary" initial={false} animate={{ width: `${forPct}%` }} /><motion.div className="h-full bg-muted" initial={false} animate={{ width: `${100 - forPct}%` }} /></div>
                      </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        {vote.status === "passed" && <Button size="sm" className="w-full"><Zap className="w-4 h-4 mr-1.5" />Execute Trade</Button>}
                      </div>
                    </div>
                  </div>
                );
