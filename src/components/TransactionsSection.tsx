...
const demoTransaction = { id: "demo-1", date: "2026-03-19", amount: 2.5, token: "SOL", from_wallet: "7xKX...3sU", type: "Payment Received", status: "confirmed", signature: "4xAb...9kL" };

const TransactionsSection = () => {
  // TODO: fetch real transactions from DB
  const realTransactions: typeof demoTransaction[] = [];
  const hasReal = realTransactions.length > 0;
  const displayTxs = hasReal ? realTransactions : [demoTransaction];
      ...
      <div className="rounded-2xl border border-border bg-card overflow-hidden">
        ...
        <div className="divide-y divide-border">
          {displayTxs.map((tx) => {
            ...
            return (
              <div key={tx.id} className={`grid grid-cols-1 md:grid-cols-8 gap-2 md:gap-4 px-5 py-4 hover:bg-secondary/30 transition-colors items-center ${isDemo ? "opacity-60" : ""}`}>
