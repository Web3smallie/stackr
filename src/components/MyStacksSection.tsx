...
import { Plus, Link2, Copy, ExternalLink, QrCode, Check, Download } from "lucide-react";
...
const demoStacks = [{ id: "1", title: "Support My Music", slug: "musicdave", description: "Help me create my next album", accepted_tokens: ["SOL", "USDC"], suggested_amounts: [1, 5, 10, 25], is_active: true }];
...
const MyStacksSection = () => {
  ...
  const [form, setForm] = useState({ title: "", slug: user?.username || "", description: "", accepted_tokens: ["SOL", "USDC", "USDT", "BAGS"] as string[], suggested_amounts: "1, 5, 10, 25", is_private: false });

  const previewLink = useMemo(() => `getstackr.app/${form.slug || user?.username || "username"}`, [form.slug, user?.username]);
  ...
  const createStack = () => {
    ...
    toast({ title: "Payment page created", description: previewLink });
    setShowCreate(false);
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <div className="rounded-2xl border border-primary/30 bg-card p-6 mb-6 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-4">
              ...
              <Input placeholder="Username / slug" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} className="bg-secondary border-border" />
            </div>
            ...
            <Button className="w-full" onClick={createStack}>Create Payment Page</Button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        {demoStacks.map((stack) => (
          <div key={stack.id} className="rounded-2xl border border-border bg-card p-6 relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-accent/5 opacity-70" />
            <div className="relative z-10">
              <div className="flex items-start justify-between mb-3"><div><h3 className="font-display text-lg font-bold text-foreground">{stack.title}</h3><p className="text-xs text-muted-foreground mt-0.5">{stack.description}</p></div><Badge variant="outline" className="bg-primary/10 text-accent border-primary/20">Active</Badge></div>
